...
void init_uart_pins(UART_Type *ptr)
{
    if (ptr == HPM_UART0) {
        ...
        HPM_IOC->PAD[IOC_PAD_PF06].FUNC_CTL = IOC_PF06_FUNC_CTL_UART1_RXD;
        HPM_IOC->PAD[IOC_PAD_PF07].FUNC_CTL = IOC_PF07_FUNC_CTL_UART1_TXD;
        ...
        HPM_IOC->PAD[IOC_PAD_PZ02].FUNC_CTL = IOC_PZ02_FUNC_CTL_UART4_DE;
        HPM_BIOC->PAD[IOC_PAD_PZ01].FUNC_CTL = BIOC_PZ01_FUNC_CTL_SOC_PZ_01;
        HPM_IOC->PAD[IOC_PAD_PZ01].FUNC_CTL = IOC_PZ01_FUNC_CTL_UART4_RXD;
        HPM_BIOC->PAD[IOC_PAD_PZ00].FUNC_CTL = BIOC_PZ00_FUNC_CTL_SOC_PZ_00;
        HPM_IOC->PAD[IOC_PAD_PZ00].FUNC_CTL = IOC_PZ00_FUNC_CTL_UART4_TXD;
        ...
        HPM_IOC->PAD[IOC_PAD_PD21].FUNC_CTL = IOC_PD21_FUNC_CTL_UART5_DE;
    } else {
        ;
    }
}
...
{
    if (ptr == HPM_I2C3) {            /* Audio /Storage/IO expand*/
    ...
    } else if (ptr == HPM_I2C1) {        /* Touch Panel */
        HPM_IOC->PAD[IOC_PAD_PD12].FUNC_CTL = IOC_PE12_FUNC_CTL_I2C1_SDA
                                            | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PD13].FUNC_CTL = IOC_PE13_FUNC_CTL_I2C1_SCL
                                            | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PD12].PAD_CTL = IOC_PAD_PAD_CTL_OD_MASK;
        HPM_IOC->PAD[IOC_PAD_PD13].PAD_CTL = IOC_PAD_PAD_CTL_OD_MASK;
    } else if (ptr == HPM_I2C0) {         /*  Camera */
        HPM_IOC->PAD[IOC_PAD_PE09].FUNC_CTL = IOC_PE09_FUNC_CTL_I2C0_SDA
                                            | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PE08].FUNC_CTL = IOC_PE08_FUNC_CTL_I2C0_SCL
                                            | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PE09].PAD_CTL = IOC_PAD_PAD_CTL_OD_MASK;
        HPM_IOC->PAD[IOC_PAD_PE08].PAD_CTL = IOC_PAD_PAD_CTL_OD_MASK;
    } else {
        ;
    }
}
...
void init_cam_pins(void)
{
    ...
    HPM_PIOC->PAD[IOC_PAD_PE11].FUNC_CTL = IOC_PE11_FUNC_CTL_GPIO_E_11;

    HPM_IOC->PAD[IOC_PAD_PA11].FUNC_CTL = IOC_PA11_FUNC_CTL_CAM0_D_2;
    HPM_IOC->PAD[IOC_PAD_PA10].FUNC_CTL = IOC_PA10_FUNC_CTL_CAM0_D_3;
    HPM_IOC->PAD[IOC_PAD_PA12].FUNC_CTL = IOC_PA12_FUNC_CTL_CAM0_D_4;
    HPM_IOC->PAD[IOC_PAD_PA13].FUNC_CTL = IOC_PA13_FUNC_CTL_CAM0_D_5;
    HPM_IOC->PAD[IOC_PAD_PA19].FUNC_CTL = IOC_PA19_FUNC_CTL_CAM0_D_6;
    HPM_IOC->PAD[IOC_PAD_PA18].FUNC_CTL = IOC_PA18_FUNC_CTL_CAM0_D_7;
    HPM_IOC->PAD[IOC_PAD_PA17].FUNC_CTL = IOC_PA17_FUNC_CTL_CAM0_D_8;
    HPM_IOC->PAD[IOC_PAD_PA16].FUNC_CTL = IOC_PA16_FUNC_CTL_CAM0_D_9;
    HPM_IOC->PAD[IOC_PAD_PA21].FUNC_CTL = IOC_PA21_FUNC_CTL_CAM0_XCLK;
    HPM_IOC->PAD[IOC_PAD_PA15].FUNC_CTL = IOC_PA15_FUNC_CTL_CAM0_HSYNC;
    HPM_IOC->PAD[IOC_PAD_PA14].FUNC_CTL = IOC_PA14_FUNC_CTL_CAM0_VSYNC;
    HPM_IOC->PAD[IOC_PAD_PA20].FUNC_CTL = IOC_PA20_FUNC_CTL_CAM0_PIXCLK;
}
#include <hpm_gpio_drv.h>
#include <hpm_gpiom_drv.h>
void init_cam_mipi_csi_pins(void)
{
    ...
    gpiom_set_pin_controller(HPM_GPIOM, GPIOM_ASSIGN_GPIOE, 3, gpiom_soc_gpio0);
    gpio_set_pin_output(HPM_GPIO0, GPIO_OE_GPIOE, 3);
    gpio_write_pin(HPM_GPIO0, GPIO_DO_GPIOE, 3, 1);
}
...
void init_gpio_pins(void)
{
    ...
    gpio_set_pin_input(HPM_GPIO0, GPIO_OE_GPIOD, 22);
    gpio_disable_pin_interrupt(HPM_GPIO0, GPIO_IE_GPIOD, 22);
...
void init_sdxc_cd_pin(SDXC_Type *ptr, bool as_gpio)
{
    if (ptr == HPM_SDXC1) {
        if (as_gpio) {
            ...
            HPM_IOC->PAD[IOC_PAD_PD16].PAD_CTL =
                    IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_DS_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1);
            HPM_GPIO0->OE[GPIO_OE_GPIOD].CLEAR = 1UL << 16;
        }
    }
}
...
void init_can_pins(MCAN_Type *ptr)
{
    if (ptr == HPM_MCAN0)
    {
        HPM_IOC->PAD[IOC_PAD_PF01].FUNC_CTL = IOC_PF01_FUNC_CTL_MCAN0_RXD;
        HPM_IOC->PAD[IOC_PAD_PF00].FUNC_CTL = IOC_PF00_FUNC_CTL_MCAN0_TXD;
        HPM_IOC->PAD[IOC_PAD_PF02].FUNC_CTL = IOC_PF02_FUNC_CTL_MCAN0_STBY;
    }
    else if (ptr == HPM_MCAN1)
    {
        HPM_IOC->PAD[IOC_PAD_PF04].FUNC_CTL = IOC_PF04_FUNC_CTL_MCAN1_RXD;
        HPM_IOC->PAD[IOC_PAD_PF05].FUNC_CTL = IOC_PF05_FUNC_CTL_MCAN1_TXD;
        HPM_IOC->PAD[IOC_PAD_PF03].FUNC_CTL = IOC_PF03_FUNC_CTL_MCAN1_STBY;
    }
    else if (ptr == HPM_MCAN2)
    {
        HPM_IOC->PAD[IOC_PAD_PF09].FUNC_CTL = IOC_PF09_FUNC_CTL_MCAN2_RXD;
        HPM_IOC->PAD[IOC_PAD_PF08].FUNC_CTL = IOC_PF08_FUNC_CTL_MCAN2_TXD;
        HPM_IOC->PAD[IOC_PAD_PE10].FUNC_CTL = IOC_PE10_FUNC_CTL_MCAN2_STBY;
    }
    else if (ptr == HPM_MCAN3)
    {
        HPM_IOC->PAD[IOC_PAD_PE14].FUNC_CTL = IOC_PE14_FUNC_CTL_MCAN3_RXD;
        HPM_IOC->PAD[IOC_PAD_PE15].FUNC_CTL = IOC_PE15_FUNC_CTL_MCAN3_TXD;
        HPM_IOC->PAD[IOC_PAD_PE13].FUNC_CTL = IOC_PE13_FUNC_CTL_MCAN3_STBY;
    }
    else if (ptr == HPM_MCAN4)
    {
        HPM_IOC->PAD[IOC_PAD_PE17].FUNC_CTL = IOC_PE17_FUNC_CTL_MCAN4_RXD;
        HPM_IOC->PAD[IOC_PAD_PE16].FUNC_CTL = IOC_PE16_FUNC_CTL_MCAN4_TXD;
        HPM_IOC->PAD[IOC_PAD_PE18].FUNC_CTL = IOC_PE18_FUNC_CTL_MCAN4_STBY;
    }
    else if (ptr == HPM_MCAN5)
    {
        HPM_IOC->PAD[IOC_PAD_PE20].FUNC_CTL = IOC_PE20_FUNC_CTL_MCAN5_RXD;
        HPM_IOC->PAD[IOC_PAD_PE21].FUNC_CTL = IOC_PE21_FUNC_CTL_MCAN5_TXD;
        HPM_IOC->PAD[IOC_PAD_PE19].FUNC_CTL = IOC_PE19_FUNC_CTL_MCAN5_STBY;
    }
    else if (ptr == HPM_MCAN6)
    {
        HPM_IOC->PAD[IOC_PAD_PE25].FUNC_CTL = IOC_PE25_FUNC_CTL_MCAN6_RXD;
        HPM_IOC->PAD[IOC_PAD_PE24].FUNC_CTL = IOC_PE24_FUNC_CTL_MCAN6_TXD;
        HPM_IOC->PAD[IOC_PAD_PE26].FUNC_CTL = IOC_PE26_FUNC_CTL_MCAN6_STBY;
    }
    else if (ptr == HPM_MCAN7)
    {
        HPM_IOC->PAD[IOC_PAD_PE30].FUNC_CTL = IOC_PE30_FUNC_CTL_MCAN7_RXD;
        HPM_IOC->PAD[IOC_PAD_PE31].FUNC_CTL = IOC_PE31_FUNC_CTL_MCAN7_TXD;
        HPM_IOC->PAD[IOC_PAD_PE29].FUNC_CTL = IOC_PE29_FUNC_CTL_MCAN7_STBY;
    }
}
...
//    HPM_IOC->PAD[IOC_PAD_PB01].FUNC_CTL = IOC_PB01_FUNC_CTL_GPIO_B_01;
    ...
    gpiom_set_pin_controller(HPM_GPIOM, GPIOM_ASSIGN_GPIOZ, 7, gpiom_soc_gpio0);
    gpio_set_pin_output(HPM_GPIO0, GPIO_OE_GPIOZ, 7);
    gpio_write_pin(HPM_GPIO0, GPIO_DO_GPIOZ, 7, 1);

    HPM_PIOC->PAD[IOC_PAD_PY00].FUNC_CTL = IOC_PAD_FUNC_CTL_ALT_SELECT_SET(3);
    HPM_IOC->PAD[IOC_PAD_PY00].FUNC_CTL = IOC_PAD_FUNC_CTL_ALT_SELECT_SET(0);
